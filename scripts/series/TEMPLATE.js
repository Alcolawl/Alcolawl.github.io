...
var finalize = "<tr><th<td width=\"5%\"><th><th>Take a Drink when:</th></tr>";                                  //Setup first row
...
function createTable() {
    $.ajax({
        ...
        success: function (xml) {
            ...
            //GULP SECTION           
            //output = output + "<tr><th width=\"5%\"></th><th>Take Two Drinks When:</th></tr>";
            for (var i = 1; i < 7; i++) {
                $(xml).find('gulp').each(function () {                                          //Gather Universal GULP rules
                    var rule = "";                                                              //Make Rule blank
                    var rule = $(this).find('rule' + String.fromCharCode(letter + i)).text();   //Find next Rule (RuleA,RuleB,etc.)
                    if (rule == "")                                                             //If there is no rule or it's blank, break the loop
                    { return false; }
            ...
            for (var i = 1; i < 7; i++) {
                $(xml).find('chug').each(function () {                                          //Gather Universal CHUG rules
                    var rule = "";                                                              //Make Rule blank
                    var rule = $(this).find('rule' + String.fromCharCode(letter + i)).text();   //Find next Rule (RuleA,RuleB,etc.)
                    if (rule == "")                                                             //If there is no rule or it's blank, break the loop
                    { return false; }
